...
public class ExpandTabView extends LinearLayout implements OnDismissListener {
	...
	private ToggleButton selectedButton;//记录被点击的那个tabItem按钮
	private ArrayList<String> mTextArray = new ArrayList<String>();
	private ArrayList<RelativeLayout> mViewArray = new ArrayList<RelativeLayout>();//点击tabitem后的弹出界面
	private ArrayList<ToggleButton> mToggleButton = new ArrayList<ToggleButton>();//上面三个开关按钮
	...
	public void setPhoto(int valuePhoto, int position) {
		if (position < mToggleButton.size()) {
			...
			//设置左边和右边的图片
			mToggleButton.get(position).setCompoundDrawablesWithIntrinsicBounds(valuePhoto,0, R.mipmap.ic_routelist_dropdown_arrow,0);
		}
	}
	...
	/**
	 * 设置tabitem的个数和初始值
	 * @param textArray 标题ArrayList
	 * @param viewArray 每个tabitem点击后显示的弹出窗口里面的选项View
     * @param photo 标题左边的图片
     */
	public void setValue(ArrayList<String> textArray, ArrayList<View> viewArray, int[] photo) {
		...
		for (int i = 0; i < viewArray.size();i++) {
			...
			RelativeLayout.LayoutParams rl = new RelativeLayout.LayoutParams(android.view.ViewGroup.LayoutParams.FILL_PARENT, maxHeight);//设置宽高
			rl.leftMargin = 10;
			rl.rightMargin = 10;
			...
			tButton.setCompoundDrawablesWithIntrinsicBounds(photo[i],0,R.mipmap.ic_routelist_dropdown_arrow,0);
			addView(tButton);
